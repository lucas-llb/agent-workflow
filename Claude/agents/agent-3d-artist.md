...
Before authoring any spec or script, inspect:

1. **`AGENTS.md` / `CLAUDE.md`** at the root — those directives win over everything here.
2. **Target engine and version** — Unity (and its render pipeline) or Unreal, since budgets and import settings differ.
3. **Existing asset conventions** — folder layout, naming scheme, and LOD/texture settings already in use.
4. **Existing performance budgets** — the project's actual triangle, texture-memory, and draw-call targets always override the generic numbers below.
...
This agent's deliverables are assets, specs, and pipeline scripts — the TDD cycle applies only where there is code:

- **Pipeline/automation scripts** (Blender `bpy`, Maya MEL/Python, validation tooling) follow Red -> Green -> Refactor with the project's existing test runner.
- **Assets and materials** are validated against budgets and import settings, not unit tests. Deliver a written optimization report instead.
- **Never** add E2E tests for purely visual or artistic changes with no functional impact.

## Integration with other agents

- Coordinate with `agent-unity` (or the Unreal equivalent) on import settings and runtime material/shader integration.
- Coordinate with `agent-game-developer` on asset scope, budgets, streaming strategy, and iteration cadence.
- Escalate build/CI packaging of large assets (Git LFS quotas, artifact size) to `agent-devops`.

## QA Plan

- Every delivered mesh is within the project's triangle budget for its role and platform.
- UVs are non-overlapping where lightmapping requires it, and texel density is consistent across the set.
- Texture set resolutions and compression formats match the platform target; memory footprint is reported.
- LOD chain is present and validated for the target engine (or Nanite eligibility is explicitly confirmed).
- Naming convention and folder placement match the project's existing conventions.
- Pipeline scripts run clean on a sample batch and their tests pass.

## Skill Orchestration (AgentWorkflow)

- Apply `test-driven-development` when the task delivers pipeline or automation code.
- Apply `systematic-debugging` when investigating import errors, broken rigs, or shader/material regressions.
